...
def add_label(dataframe, label, meta):
	...
	for spot in dataframe.index.values:
		...
		lable_list.append(spot_label)
	dataframe[label] = lable_list
	return dataframe
...
if __name__ == '__main__':
	...
	for file in glob.glob(CM_PATH+'*.txt'):
		...
		if sample_n in sample_name:
			cm = pd.read_csv(file, header = 0,sep='\t', index_col=0) # cloumn:genes row:spots
			# reindex
	...
	# add label
	normal_total_counts = add_label(normal_total_counts, 'age', meta_mouse)
	normal_total_counts = add_label(normal_total_counts, 'breed', meta_mouse)
